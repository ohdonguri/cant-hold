...
//
// `npm test` 의 밸런스 블록(고정덱 1개 · 16회 · 무시드)은 **게이트용이지 눈금이 아니다.**
// 고정덱 한 벌로는 「S2 를 매번 깨는가 자주 깨는가」를 못 가른다 — 실제로 그 8/8 을
// 35덱 표의 17/35 와 나란히 놓고 읽은 적이 있는데, 하나는 고정덱 판 단위이고 하나는
// 덱 단위라 애초에 같은 축이 아니었다. 상수를 정할 때는 여기(210판)를 쓴다.
// (표본이 8 → 16 으로 올라간 것은 게이트의 흔들림 때문이지 눈금이 되어서가 아니다.
//  근거는 tools/test.js §`const n`.)
//
// ── 왜 지표가 둘인가 ─────────────────────────────────────────
