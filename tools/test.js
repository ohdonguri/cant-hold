...
// 바꾸므로 분포가 나빠지면 여전히 빨간불이 된다.
{
  ...
  //
  // [2026-08 #35] **래칫은 2 그대로 두되, 밑에 깔린 확률이 올라간 것을 적어 둔다.**
  // 배치 정책이 붙으면서 S4 클리어율이 **1000판 실측으로 5.0% → 6.6%** 로 올랐다
  // (k=1 50/1000 · k=2 66/1000). 8판 중 3판 이상이 나올 확률은 그만큼
  // **0.58% → 1.3%** 로 두 배가 됐다. **그런데 32런을 다시 떠 본 최악값은 여전히
  // 2/8 이라**(0/8 이 19런 · 1/8 이 10런 · 2/8 이 3런) 래칫을 올릴 근거가 없다 —
  // 래칫은 「이론상 나올 수 있는 값」이 아니라 **실측 최악값**이라야 회귀를 잡는다.
  // 다만 1.3% 면 30런에 한 번쯤 3/8 이 나올 수 있으니, 한 번 봤다고 바로 올리지
  // 말고 다시 떠서 최악값이 실제로 옮겨졌는지부터 볼 것.
  ...
  known('4스테이지는 안 깨진다', s4.clears > S4_KNOWN_CLEARS,
    ...
    + '정책이 한 번 더 밀었다. 무시드 1000판 S4 클리어율 5.0% → 6.6% 라 8판 중 2판이 '
    + '9.3% 확률로 나온다(3판은 1.3%). 난이도 재조정은 별도 티켓이고, 0 으로 '
    + '돌아오면 KNOWN 에서 지우고 하드 게이트로 되돌릴 것.');
  if (s4.clears <= 1) {
