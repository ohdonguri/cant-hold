...
// 「어제 초록이던 것이 오늘 빨간불인가」뿐이고, 그 목적에는 8판이면 된다.
{
  ...
  for (const st of stageIdx) {
    ...
    // 달라진다 — 그 함정이 #37 이다.
    rows.push({ st, clears, med: w[n >> 1], w, five, p: w.reduce((a, b) => a + b, 0) / n / max });
  }

  for (const r of rows) console.log('    S' + (r.st + 1) + '  클리어 ' + r.clears + '/' + n
    + '  중앙 w' + r.med + '  진도 p ' + r.p.toFixed(2));
  ...
  known('4스테이지는 안 깨진다', s4.clears > S4_KNOWN_CLEARS,
    ...
    + '정책이 한 번 더 밀었다. 무시드 **1000판** S4 클리어율 5.0% → 6.6% 라 8판 중 '
    + '2판이 9.3% · 3판이 1.25% 확률로 나온다 — 래칫을 2 에서 3 으로 올린 것이 그 '
    + '실측이다(임계는 안 건드렸다). 난이도 재조정은 별도 티켓이고, 0 으로 '
  ...
  // 찍혀 「뒤집혔다」로 읽힌다 — 이 줄의 요지가 「그 차이는 안 읽힌다」인데.
  const dp = d => { const v = Math.round(d * 100) / 100; return (v >= 0 ? '+' : '') + v.toFixed(2); };
  const steps = rows.slice(1).map((r, i) => 'S' + (rows[i].st + 1) + '→S' + (r.st + 1)
    + ' Δp ' + dp(r.p - rows[i].p));
