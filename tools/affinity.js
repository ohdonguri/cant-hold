...
//
// ── 허용이 3종이면 이 도구는 **아무것도 못 잰다** (#56) ──────────
// 허용이 `DECK_SIZE` 와 같으면 덱이 `C(3,3) = 1` 개다. 이 자는 「든 덱 평균 − 안 든 덱
// 평균」인데 **한쪽 무리가 통째로 비므로** 어느 열도 뺄 것이 없다 — 그래서 그 행은
// 일곱 칸이 전부 `─` 다. **0 으로 찍으면 안 된다**: 0 은 「선호 없음」으로 읽히는데
// 여기는 선호가 없는 게 아니라 **잴 자가 없는** 것이다(이 파일 머리 §포화된 판과
// 같은 규칙이고, #50 에서 이미 같은 처리를 했다).
//
// 그 판의 강제는 이 표가 아니라 **구조**가 보증한다 — 허용 3종 · 공격 1 + 오라 2 ·
// `DECK_SIZE` 와 같음. `tools/test.js` 「강제」 블록이 그 셋을 단언한다.
//
...
console.log('분할은 덱 수에 딸린다 — 35덱 판은 15 대 20, 4덱 판은 3 대 1, 1덱 판은 1 대 0 이다.');
console.log('`포화`/`무신호`/`1덱` 이 붙은 행은 아무것도 안 재고 있다 — 0 을 「선호 없음」으로 읽지 마라.');
console.log('* 는 공격 타워(KINDS[k].group === "attack"). 자리 선택이 성능인 것은 이쪽이다.');
console.log('`─` 는 뺄 것이 없어 못 잰 칸이다. 0 과 다르다. 두 가지가 여기 들어온다 —');
console.log('  ① 그 판이 아예 안 받는 종류(allowKinds)  ② 덱이 하나뿐이라 「안 든 덱」이 없는 판(1덱)');
console.log('1덱 판은 일곱 칸이 전부 `─` 다. 그 판의 강제는 이 표가 아니라 구조가 보증한다');
console.log('(허용 3종 = DECK_SIZE → 덱이 유일 → 그 타워가 반드시 들어간다. npm test 「강제」 블록).');
console.log('');
...
for (const r of out) {
  // 덱이 하나뿐인 판은 **신호폭이 정의상 0** 이라 그냥 두면 `무신호` 가 붙는데,
  // 그건 「덱끼리 안 갈린다」는 뜻이라 여기서는 틀린 말이다 — 갈릴 덱이 없다.
  // 표시를 따로 둔다(#56).
  const mark = r.decks === 1 ? '1덱'
    : r.clearRate >= CLEAR_HI ? '포화' : r.spread < SPREAD_LO ? '무신호' : '';
  console.log(
