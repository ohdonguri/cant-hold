...
const capture = async (browser) => {
  ...
  // 안 사라지고 **탭을 기다린다**(index.html 의 머리 주석 — 토스 스플래시와의 경주를
  // 기기에서 두 번 지고 내린 결론이다). 안 걷으면 백스톱 15초까지 모든 컷이 통째로
  // 로고 화면이다. 이 하네스가 보는 것은 게임 화면이므로 찍기 전에 없앤다.
  ...
  // 짚는다 — `WAVES` 가 30줄이던 시절 **표 밖을 짚는 유일한 컷**이었고, 진짜 마지막
  // 줄(w30)과 그 되풀이(w31·w32)가 한 화면에 같이 찍혔다. 고치기 전에는 여기서
  // `groups is not iterable` 로 화면이 통째로 죽었고(`drawForecast` 만 되풀이 규칙을
  // 안 쓰고 있었다), `npm test` 는 렌더를 안 보므로 **한 줄도 안 걸렸다.**
  //
  // **[#95] 이 컷이 재는 것이 바뀌었다.** 표가 46줄이 되면서 w30·w31·w32 는 전부
  // 표 안이다 — 그리고 `drawForecast` 가 `w > CFG.WAVE_MAX` 에서 끊으므로 **어느
  // 판에서도 예고가 표 밖을 짚을 수 없다**(가장 긴 판이 46웨이브이고 표가 46줄이다).
  // 「표 밖」을 재던 자리는 없어졌고, 대신 **되풀이의 증상**을 잰다: 표가 다시 30줄로
  // 잘리면 w30·w31·w32 가 **세 칸 모두 같은 편성**으로 찍힌다. 그래서 이 컷은
  // 「예고 세 칸이 서로 다른가」를 본다 — **웨이브를 30 이하로 내리지 마라. 내리면
  // 세 칸이 전부 표 앞쪽이라 아무것도 안 잰다.**
  //
  // 볼 것:
  //   ① **`웨이브 28 / 38` 이 안 깨지는가** — 「/ 38」이 두 자리라 제목 오른쪽에
  //      붙는 자리가 30 일 때와 같다. 골드·정수/관문 줄과 안 겹쳐야 한다
  //   ② **예고 세 칸이 다 그려지고 서로 다른가** — 세 칸이 같은 편성이면 `WAVES`
  //      되풀이 구간에 들어간 것이고, 그건 후반이 설계보다 무르다는 뜻이다(#95).
  //      셋째 칸이 아예 비면 그건 `waveGroups()` 가 안 걸린 것이다
  ...
  await page.evaluate(() => {
    ...
    tuteMerged = true;
    state.gold = 99999;
    // 웨이브 29 — 예고가 w30·w31·w32 를 짚는다. **표가 30줄로 잘리면 셋이 같아지는
    // 자리다**(#95 전에는 실제로 그랬다). 지금은 w30 총정리 · w31 중갑+면역 ·
    // w32 마갑+면역 으로 셋이 서로 달라야 한다.
    state.wave = 29;
  ...
  await shot('26-jallogi', () => {
    ...
    // **예고 세 칸이 옛 표(30줄)를 넘어가는가.** 넘어가지 않으면 이 컷은 멀쩡히
    // 나오는데 아무것도 안 잰다 — 그 자리를 잠그는 줄이다.
    if (state.wave + 3 <= 30)
      return '예고가 옛 표(30줄) 안에서 끝난다: w' + (state.wave + 3);
    // **세 칸이 서로 다른가.** 표가 30줄로 잘리면 되풀이 규칙이 걸려 셋이 같아진다.
    const fc = [1, 2, 3].map(i => waveGroups(state.wave + i));
    if (fc.some(g => !g)) return '예고 웨이브의 편성이 없다: w' + (state.wave + 1) + '~';
    const key = fc.map(g => g.map(([k, n]) => k + n).join('+'));
    if (new Set(key).size !== 3) return '예고 세 칸이 되풀이다: ' + key.join(' / ');
