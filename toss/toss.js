...
// ── 위쪽 조작 자리 ───────────────────────────────────────────
// 토스는 미니앱 위에 **X(닫기)와 더보기(⋯)** 를 얹는다. 게임은 화면 오른쪽 위에 보유
// 골드를 적는데(`view.w - 16, y=16`) 그 자리가 정확히 겹쳐서 **골드가 안 읽힌다**(신고).
//
// **SDK 에 이 높이를 주는 API 가 없다.** `SafeArea.get()` 은 기기 안전영역(상태표시줄·
// 노치)이고 그 버튼들은 그 아래에 따로 얹히는 것이라 거기 안 들어간다. 그래서 상수로
// 둔다 — 값을 고치는 자리가 여기 한 곳이라는 것이 이 상수의 값이다.
//
// 게임은 이 값을 `window.__topReserve` **하나로만** 안다. `__bottomReserve` 와 같은
// 통로이고(index.html `resize`), 웹판·안드로이드에서는 아무도 안 세우므로 0 이다.
const TOSS_TOP_CONTROLS = 48;
...
// 위쪽 조작 자리를 게임에 알리고 한 번 다시 잰다. **부팅에서 한 번이면 된다** —
// 이 값은 화면 회전이나 배너처럼 오가는 값이 아니라 토스 셸의 성질이다.
window.__topReserve = TOSS_TOP_CONTROLS;
relayout();
