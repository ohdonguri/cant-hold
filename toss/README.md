...
3. **광고 그룹 ID** — `toss.js` 상단 `AD` 의 `fullScreen` · `banner` 를 콘솔 발급값으로
   ...
   내고 그게 판마다 반복된다). **둘 다 채웠다** — 배너 `ait.v2.live.ce271d2…`(스테이지
   목록 · 덱 선택 · 일시정지에서만 뜬다), 전면 `ait.v2.live.498fb6…`(판 클리어·실패 때만).
   비는 키가 생기면 부팅 로그가 그 이름을 찍는다.
4. **버전을 채널 간에 맞춘다** — 이 폴더의 `package.json` 의 `version` 이 미니앱 채널의
   버전이다. 웹판에는 버전 표시가 없어서 대조할 상대가 없지만, **안드로이드 채널에는
   있다** — `android/app/build.gradle` 의 `versionName` 과 같은 값으로 둔다.
   지금은 둘 다 `1.0.1` 이다.
