...
- **스테이지 11개.** 맵마다 좋은 자리가 다르다. 깨면 다음이 열린다. ⑦⑧⑨ 는 **쓸 수 있는 타워를 제한한다** — ⑦ 은 4종만 주고(오라 둘을 고른다), ⑧⑨ 는 3종만 줘서 **덱이 통째로 하나**다. ⑩⑪ 은 제한이 없고 **레인이 셋**이다. ⑪ 빗장은 **자음(ㅂ)을 모티프로 쓴 첫 판**이다 — 글자를 그대로 그리지는 않는다(DESIGN §자음 모티프)
