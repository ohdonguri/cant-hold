...
- **스테이지 7개.** 맵마다 좋은 자리가 다르다. 깨면 다음이 열린다. 마지막 판은 **쓸 수 있는 타워를 제한한다**
...
npm run curve 스테이지 난이도 곡선 (판당 35덱 x 6회 = 210판). **난이도 눈금의 정본이다**.
             덱을 제한하는 판(`allowKinds`)은 4덱 x 6회 = 24판이라 **표본이 다르다** —
             행 끝 태그에 덱 수가 찍히고, 다른 표본끼리 나란히 읽으면 안 된다
npm run affinity 판이 어느 타워를 좋아하는가 (판당 35덱 x 8회 · 제약 판은 4덱). 난이도와
             **직교하는 자**다 — 같은 판 안에서 어느 타워가 값을 하는지를 잰다. 포화된
             판은 행에 `포화` 가 붙는다. 거기서는 0 이 「선호 없음」이 아니라 「못 잰다」다.
             `─` 는 그 판이 아예 안 받는 종류다(0 과 다르다)
