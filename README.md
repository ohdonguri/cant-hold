...
초원·숲, 협곡·절벽, 여울·습지, 설산·고개, 유적·환상석, 성문·폐성,
황야·채석장에 각각 다른 긴장감 있는 브금이 나온다. 테마별 32마디 원곡에
세 가지 편곡을 사용하고, 같은 테마의 스테이지에서도 리듬과 선율을 바꾼다.
숲은 12/8 북·피리, 협곡은 빠른 브레이크비트, 습지는 느린 덥 베이스,
설산은 현악 반복음, 유적은 5/4 금속 종, 성문은 행진 북, 황야는 거친
전자 베이스로 박자·템포·악기부터 구분한다.
재도전하면 시작 구절과 편곡이 순환한다(페이지를 새로 열면 초기화).

웨이브 진행률 30%부터 테마 고유의 타악기와 응답구를 더하고, 70%부터
화음과 보조 리듬을 쌓는다. 준비 시간에는 악기를 줄인다. 배속과 관계없이 템포는 유지하며,
일시정지·화면 이탈·결과 화면에서는 멈춘다. 정지 화면의 **소리**로 효과음과
브금을 함께 끌 수 있고 설정은 기기에 저장된다.

별도 음원 다운로드 없이 기존 Web Audio 효과음 엔진과 함께 합성한다.
`npm test`에 악보·스케줄러 검사가 포함되어 있다. 실제 Chromium 재생과 7개 곡의
오디오 파형은 `node tools/bgm-browser-check.js`로 확인한다. 출력 폴더를 인자로
주면 테마별 8초씩 이어 붙인 `theme-bgm-sampler.wav`도 만든다.
...
- **빈 칸을 누르면** 그 칸 둘레 10시·12시·2시에 덱 아이콘 셋이 뜬다
  - **아이콘을 한 번 탭하면 바로 설치된다**
  - **아이콘을 꾹 누르면** 사거리와 설치 위치를 미리 본다. 손을 떼도 설치되지 않는다
  - 취소는 **아이콘 밖 탭** 또는 **ESC**
- 타워를 **다른 타워 위로 끌면** 합성. 같은 종류·같은 성급만. **웨이브 중에도 된다**
- 타워를 **꾹 누르는 동안** 사거리 확인. **짧게 탭하면** 정보, 판매/분기 재선택
...
클라우드 저장은 매번 Firestore 트랜잭션으로 서버의 최신 기록을 읽고 병합한다.
해금·최고 웨이브는 큰 값, 클리어 여부는 OR로 보존하므로 새 기기나 오래 켜 둔 탭이
낡은 기록으로 덮어쓰지 않는다. 읽기 실패·오프라인 상태에서는 서버에 쓰지 않으며,
다음 저장 때 다시 시도한다. 같은 탭의 요청은 순서대로 처리한다.
서버 이어하기는 첫 동기화에서 목록·덱 선택 화면에 있을 때 가져오고, 이미 시작한 판은 유지한다.

이미 기록이 사라졌다면 기존 기기/탭의 로컬 기록이나 서버 백업이 남아 있는지 먼저 확인한다.
이 수정은 남아 있는 기록의 재손실을 막는 것으로, 지워진 데이터를 자동 복구하지 않는다.
구버전 탭은 기존 저장 코드를 계속 쓰므로 업데이트 후에는 열려 있던 게임 탭도 새로고침해야 한다.
...
스테이지·덱 선택 화면과 일시정지 메뉴의 **타워 도감**에서 7종 타워의 3성 분기, 각 분기로 이어지는 5성 특성, 7성 공통 특성을 미리 볼 수 있습니다. 모든 타워는 최대 7성까지 성장하며, 7성에서는 공통 특성 7개 중 무작위 3개를 제시하고 하나를 선택합니다. 도감 설명은 실제 게임 데이터와 연결됩니다.

상단 체력은 하트 15개로 표시합니다. 하트 하나는 체력 2, 반쪽은 체력 1이며 정확한 현재/최대 체력도 함께 표시합니다. `npm run test:guide`는 모바일 도감·뒤로가기·일시정지 복원을 브라우저에서 검사합니다.
