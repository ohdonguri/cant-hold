...
- **스테이지 14개.** 맵마다 좋은 자리가 다르다. 깨면 다음이 열린다. ⑦⑧⑨ 는 **쓸 수 있는 타워를 제한한다** — ⑦ 은 4종만 주고(오라 둘을 고른다), ⑧⑨ 는 3종만 줘서 **덱이 통째로 하나**다. ⑩~⑭ 는 제한이 없고 **레인이 셋**이다. 그중 ⑪ 빗장 · ⑫ 턱 · ⑬ 재 · ⑭ 후미는 **자음(ㅂ·ㅌ·ㅈ·ㅎ)을 모티프로 쓴 계열**이다 — 글자를 그대로 그리지는 않는다(DESIGN §자음 모티프)
...
- **빈 칸을 누르면** 그 칸 둘레 10시·12시·2시에 덱 아이콘 셋이 뜬다. **아이콘을 누르면 바로 선다**(탭 두 번)
  - 아이콘을 **길게 누르면** 그 종류의 **사거리만** 보인다. 손을 떼도 안 지어진다
  - 취소는 **아이콘 밖 탭** 또는 **ESC**. 누른 채 옆으로 밀어도 안 지어진다
